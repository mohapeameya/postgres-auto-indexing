...
{    
    char sourcequery[1000];
    strcpy(sourcequery,queryDesc->sourceText);
	int counter = 0;
    for (counter=0;counter<strlen(sourcequery);counter++){
        if(sourcequery[counter]==','){
            sourcequery[counter]=' ';
        }
    }
	...
	if (queryDesc->operation == CMD_SELECT) {
		if (strstr(queryDesc->sourceText, "explain") == NULL && strstr(queryDesc->sourceText, "pg_") == NULL) {
			const FILE *fptr1 = fopen("/home/ashish/Desktop/log/explain_file_match1.csv", "a");
			char *relnamecontinue;
			relnamecontinue = strstr(es->str->data,"Seq Scan");
			char *start_ = es->str->data;

			char relname[30];
			char cost[10];
			char attr[50];
			char *totalcostcontinue;
			char *attrcontinue;
			char alias[30];
			char *aliascontinue;
			char *colfind;
			while(relnamecontinue != NULL){
			relnamecontinue = strstr(relnamecontinue,"Relation Name");
			if (relnamecontinue == NULL)
			    break;
			memset(relname, '\0',30);
			char c;
			int i = 0;
			int j = 0;
			while (1){
			    if(relnamecontinue[i] != ':'){
			         i++;
			    }else{
			         break;
			    }
			}
			while(c=relnamecontinue[i]){
			    if (c == ':' || c==' ' || c=='\"'){
			         i++;
			         continue;                         
			    }
			...
			//alias
			aliascontinue = strstr(relnamecontinue,"Alias");
			...
			totalcostcontinue = strstr(aliascontinue,"Total Cost");
			memset(cost, '\0',10);
			i = 0;
			j = 0;
			while (1){
			    if(totalcostcontinue[i] != ':'){
			         i++;
			    }else{
			         break;
			    }
			}
			while(c=totalcostcontinue[i]){
			    if (c == ':' || c==' ' || c=='\"'){
			         i++;
			         continue;                         
			    }
			...
			attrcontinue = strstr(totalcostcontinue,"Filter");
			if (attrcontinue==NULL){
	    		//finding columns when no filter
				...
				relnamecontinue = strstr(relnamecontinue, "Seq Scan");     
				Name db;
				db = (Name) palloc(NAMEDATALEN);
				namestrcpy(db, get_database_name(MyDatabaseId));
				fprintf(fptr1, "%s,%s ,%s, %s, %s,%s\n", relname,alias ,cost, attr, db, sourcequery); 
				pfree(db);
            }
	        fptr = fopen("/home/ashish/Desktop/log/explain_file_match.txt", "a");
			fprintf(fptr, "%s@", es->str->data);
			fflush(fptr);
			fclose(fptr);
            fflush(fptr1);
            fclose(fptr1);
